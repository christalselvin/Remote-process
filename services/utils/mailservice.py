...
SMTP_FROM = "saravanammal.m@autointelli.com"
template_html = """
                                                                                 ...
                                                                                 <div class="contentEditableContainer contentTextEditable content-footer">
                                                                                    <div class="contentEditable" align='center'>
                                                                                       <p  style='text-align:left;color:#09090c;font-weight:normal;line-height:25px;'>
                                                                                          ...
                                                                                          <span style='font-weight:bold;'>IRESS</span><br><br>
                                                                                          <br>
                                                                                      </p>
                                                                                    </div>
                                                                                 </div>
...
def getConfig():
    SMTP_COMM_TYPE, SMTP_IP, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD = "", "", "", "", ""
    smtp=ConfigModel.query.filter(ConfigModel.configname=='SMTP').first()


    if smtp is not None:
        SMTP_COMM_TYPE = smtp.communicationtype
        SMTP_IP = smtp.configip
        SMTP_PORT = smtp.configport
        SMTP_USERNAME = smtp.username

        SMTP_PASSWORD = decode('auto!ntell!',smtp.password)

    return SMTP_COMM_TYPE, SMTP_IP, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD


def sendmail(sSubject, lTo, lCC, sBody):
    SMTP_COMM_TYPE, SMTP_IP, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD = getConfig()
    ...
    if s.has_extn('STARTTLS'):
        s.starttls()
        s.ehlo()
    if (SMTP_USERNAME != "" and SMTP_PASSWORD != "" and SMTP_USERNAME != "None" and SMTP_PASSWORD != "None"):
        try:
...
#     notes = "Thanks for registering to http://autointelli/ <BR/>User this password <B>IRESS@123</B> to do first login</BR>" + notes
#     sendmail("New User Registration", ["dinesh@autointelli.com"], ["dinesh@autointelli.com"], notes)
#     sendmail("New User Registration", ["dinesh@autointelli.com"], ["dinesh@autointelli.com"], "Thanks for registering to http://autointelli/ <BR/>User this password <B>NxtGen@123</B> to do first login")